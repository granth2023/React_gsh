...
function Safecracker({ modeToggle, pColor, handleNavigation }) {
  ...
  return (
    ...
    >
      <h2 className="text-23xl text-center sm:text-8xl font-gothic ">
        Safecracker
      </h2>
      ...
      {["Synopsis", "Dream Casting", "Writer's Note"].map((section) => (
        <React.Fragment key={section}>
          <h3
            className="text-lg sm:text-4xl font-gothic font-bold mt-12 cursor-pointer text-center"
          ...
          {expandedSections.includes(section) && (
            <div
              ...
              style={{
                ...
                borderRadius: "5px",
                boxShadow:
                  "0 10px 15px rgba(0, 0, 0, 0.2), 0 4px 6px rgba(0, 0, 0, 0.25)",
                transition: "all 0.3s ease-out",
              }}
            >
              {section === "Synopsis" && (
                <p
                  className="font-gothic text-justify mt-4 sm:text-xl "
                  style={{ maxWidth: "80%", margin: "auto" }}
                >
                  Believed only as an old Hollywood tale, the bank vault
                  underneath the Hollywood sign becomes the location for the
                  world's greatest safecracker and his crew's biggest and final
                  score. Is the vault even real? How will they get in? Who owns
                  it? And will they make it out alive?
                </p>
              )}
              {section === "Dream Casting" && (
                <p>
                  Imagine an ensemble cast featuring dynamic actors known for
                  their compelling performances in action and drama genres, such
                  as John Doe, Jane Doe, and others, bringing this thrilling
                  heist story to life.
                </p>
              )}
